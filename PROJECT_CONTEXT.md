...
- 支持创建、关联和展开 Markdown 内容文件；内容卡显示摘要、字数、清单进度和关联任务数。
- 支持内容快速编辑、预览、重命名、重绑、收起和事务式删除。
- 创建、编辑和部分状态切换复用 Tasks API；关系与其他元数据由安全写回服务修改 Markdown。
- 跨文件写入会先计算全部变更；部分文件写入失败时尝试恢复已写入文件。
- 快速编辑检测外部修改并阻止静默覆盖；地图刷新使用代次校验阻止旧结果覆盖新状态。
...
- `npm test`（或 `pnpm test`）会自动发现并运行 TypeScript 核心测试，覆盖解析、构图、布局、迁移、
  短链接、嵌套写回与回滚逻辑。
- 修改后至少运行 `npm run check`（或 `pnpm run check`），并在真实测试 Vault 中验证涉及写回的交互。
...
- `src/services/ContentDocuments.ts`：内容文件解析、创建、移动、注册和关系边。
- `src/services/DocumentMetadata.ts`：内容路径、摘要、字数和清单统计。
- `src/services/PluginData.ts`：插件数据迁移及来源路径重命名。
- `src/layout/LayeredLayout.ts`：稳定的从左到右分层布局。
- `src/layout/DocumentLayout.ts`：任务与内容卡的归属布局。
- `tests/run.mjs`：测试发现、临时编译、执行、汇总和清理入口。
- `tests/smoke.ts`：无外部测试框架的核心逻辑 smoke test。
...
- 修改等级：L3 数据安全与 0.4.0 内容节点完善。
- 修复摘要：保护嵌套任务结构；兼容短 WikiLink 写回；快速编辑防丢失；内容删除和重绑批量回滚；
  异步刷新代次控制；无 ID 坐标迁移；取消/自定义状态筛选；配置清理和依赖锁定。
- 验证方式：TypeScript 类型检查、核心 smoke test、生产 bundle、生成文件一致性和 Git diff 审查。
